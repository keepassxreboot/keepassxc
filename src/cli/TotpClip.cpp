...
TotpClip::TotpClip()
{
    name = QString("totp-clip");
    description = QObject::tr("Copy an entry's TOTP to the clipboard.");
}
...
int TotpClip::execute(const QStringList& arguments)
{
    ...
    if (args.size() != 2 && args.size() != 3) {
        out << parser.helpText().replace("keepassxc-cli", "keepassxc-cli totp-clip");
        return EXIT_FAILURE;
    }
...
int TotpClip::clipEntry(Database* database, const QString& entryPath, const QString& timeout)
{
    ...
    int exitCode = Utils::clipText(entry->totp());
    if (exitCode != EXIT_SUCCESS) {
        return exitCode;
    }

    outputTextStream << QObject::tr("Entry's TOTP copied to the clipboard!") << endl;
