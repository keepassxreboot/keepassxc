...
namespace FdoSecrets
{
    ...
    Item* Collection::onEntryAdded(Entry* entry)
    {
        ...
        Item* item = m_entryToItem.value(entry);
    ...
    bool Collection::doDeleteItem(const DBusClientPtr& client, const Item* item) const
    {
        ...
        bool permanent = inRecycleBin(item->backend()) || !m_backend->metadata()->recycleBinEnabled();
        return plugin()->requestEntriesRemove(client, m_name, {item->backend()}, permanent);
    }
    ...
    bool Collection::inRecycleBin(Group* group) const
    {
        ...
        if (!m_backend->metadata()) {
            return false;
        }

        auto recycleBin = m_backend->metadata()->recycleBin();
        if (!recycleBin) {
            return false;
        }
