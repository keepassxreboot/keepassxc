...
#ifdef WITH_XC_TOTPSEED_QRCODE
    m_actionMultiplexer.connect(m_ui->actionEntryTotpSeed, SIGNAL(triggered()),
            SLOT(showTotpSeed()));
#endif
...
#ifdef WITH_XC_TOTPSEED_QRCODE
            m_ui->actionEntryTotpSeed->setEnabled(singleEntrySelected && dbWidget->currentEntryHasTotp());
#else
            m_ui->actionEntryTotpSeed->setVisible(false);
#endif
