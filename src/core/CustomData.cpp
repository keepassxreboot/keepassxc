...
const QString CustomData::FdoSecretsClientPrefix = QStringLiteral("FDO_SECRETS_CLIENT_");
...
{
    return key.startsWith(BrowserKeyPrefix) || key.startsWith(FdoSecretsClientPrefix) || key == Created
           || key == FdoSecretsExposedGroup || key == CustomData::RemoteProgramSettings;
}
