...
const QString CustomData::CloudSyncSettings = QStringLiteral("KPXC_CLOUD_SYNC_SETTINGS");
...
bool CustomData::isProtected(const QString& key) const
{
    return key.startsWith(BrowserKeyPrefix) || key == Created || key == FdoSecretsExposedGroup
           || key == CustomData::RemoteProgramSettings || key == CustomData::CloudSyncSettings;
}
