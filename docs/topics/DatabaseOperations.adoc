...
// end::advanced[]
=== Search
KeePassXC provides a robust search that enables you to find specific entries in the databases using different modifiers, wild card characters, and logical operators. By default, search considers the following fields when matching your query: Title, Username, Password, URL, Notes, Additional Attributes, Attachments or Tags. However, protected fields (password, specified additional attributes) are only searched when the corresponding option is set. To narrow your search to specific fields, you can use the search syntax described below.
...
* Entry Feature (has:totp, has:passkey, has:expiration)

Entries with empty fields can be searched for with, e.g., `u:`.
Values in specific additional attributes can be searched by adding an underscore, e.g., `_customField1:value` searches for `value` only in field `customField1`.
Note that also protected values are found regardless of the corresponding option, when explicitly searching the password field (e.g., `p:secret`).
...
|`+user:johnsmith -url:www.google.com notes:"secret note*"`
|Search the username field for exactly johnsmith, the URL must not contain www.google.com, and notes contains 'secret note*'.
...
|`!u:`
| Searches for entries with non-empty username.
...
==== Search Options
The following options are available to affect the search:
* *Include protected fields*: Include protected fields in default search
* *Case sensitive*: Set search terms to be case-sensitive
* *Regular expression*: Search terms are handled as regular expressions
* *Limit search to selected group*: Only search within selected group
* *Press Enter to search*: Instead of automatic beginning of search, you have to press enter
