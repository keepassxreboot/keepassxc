...
*(B)* KeePassXC is running but current database is not connected to it +
*\(C)* Connected to KeePassXC, but database is locked +
*(D)* Connected to KeePassXC and ready to use

If the normal icon *(D)* is shown with a number, it indicates the number of credentials found for the current site.
+
