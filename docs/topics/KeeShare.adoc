...
KeeShare does not verify signatures on imported share containers. A signature is still produced when exporting a `.kdbx.share` container, but it is not checked at import time. The trust model relies entirely on filesystem permissions of the share container's storage location and the share password. Any party with write access to the share container path who knows the share password can substitute the database, and entries with newer modification timestamps will overwrite local entries during the merge. The container must be placed at a location which is only writable by parties whose data you trust.
// end::content[]
